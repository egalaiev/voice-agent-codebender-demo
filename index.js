...
#### Ending the call:
Once the conversation is finished or if you bid each other farewell, use the \`end_call\` function to end the call.
`
...
fastify.register(async (fastify) => {
  fastify.get('/media-stream', { websocket: true }, (connection, req) => {
    ...
    let session = sessions.get(sessionId) || { transcript: '', streamSid: null, isTerminating: false };
    ...
    openAiWs.on('message', (data) => {
      try {
        ...
        // Handle function calls (end call)
        if (response.type === 'response.function_call_arguments.done') {
          console.log("Function called:", response);
          const functionName = response.name;

          if (functionName === 'end_call') {  // If the end_call function is called
            const endCallInstruction = {
              ...
              item: {
                ...
                content: [
                  {
                    type: 'input_text',
                    text: 'If you have already said goodbye, remain silent. Otherwise, say: "It was a pleasure speaking with you. Please do not hesitate to reach out anytime. Goodbye!"'
                  }
                ]
              }
            };
            openAiWs.send(JSON.stringify(endCallInstruction));
            openAiWs.send(JSON.stringify({ type: 'response.create' }));
            ...
            setTimeout(() => {
              ...
              twilioClient.calls(callSid)
                ...
                .catch(error => console.error('Error terminating call:', error));
            }, 6000);
          }
        }
      } catch (error) {
    ...
    connection.on('message', (message) => {
      if (session.isTerminating) {
        return; // Ignore all incoming messages during termination
      }
      try {
      ...
      const timeoutInstruction = {
        ...
        item: {
          ...
          content: [
            {
              type: 'input_text',
              text: 'Limit reached, say this to user: "I need to wrap up our call now. Have a wonderful day!"'
            }
          ]
        }
      };
      openAiWs.send(JSON.stringify(timeoutInstruction));
      openAiWs.send(JSON.stringify({ type: 'response.create' }));
...
async function createNote(contactId, transcript) {
  try {
    ...
    const createNoteResponse = await axios({
      method: 'POST',
      url: 'https://rest.gohighlevel.com/v1/contacts/' + contactId + '/notes/',
      headers: {
    ...
    console.log(`Note created for contact ID ${contactId} in GHL.`);
